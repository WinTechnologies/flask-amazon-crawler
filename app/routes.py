...
def index():
    ...
    return render_template('index.html', host_url=host_url, form=search_form, count=-1)
...
def uploader():
    ...
    if request.method == 'POST' and request.files['input']:
        ...
        return str(count) + ' crawlers are running in background, you can check them to refresh the page. <a href="/" class="btn btn-primary">Refresh</a>'
    else:
        redirect('/')
...
@bp.route('/excel/<string:from_date>/<string:to_date>', methods=['GET'])
